...
// Перо для рисования графика
		graphicsStroke = new BasicStroke(5.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 20.0f,
				new float[] {10,5,5,5,10,5,10}, 1.0f);// Перо для рисования осей координат
...
// Отобразить график
	...
	Boolean even(double y) {
		...
		Integer integer_y = (int) value_y;
		String string_y = integer_y.toString();
		for (int i = 0; i < string_y.length(); i++) {
			Int_part = (int) (integer_y / (Math.pow(10, (string_y.length() - i))));
			if (!(((int) (Int_part / 2) + (int) (Int_part / 2)) == Int_part))
				is_even = false;
...
// Выбрать красный цвет для закрашивания маркеров внутри
		...
		for (Double[] point : graphicsData) {
			...
			marker.moveTo(center.getX() - 5.5, center.getY());
			marker.lineTo(center.getX() - 5.5, center.getY() + 2.75);
			marker.lineTo(center.getX() - 2.75, center.getY() + 5.5);
			marker.lineTo(center.getX() + 2.75, center.getY() + 5.5);
			marker.lineTo(center.getX() + 5.5, center.getY() + 2.75);
			marker.lineTo(center.getX() + 5.5, center.getY() - 2.75);
			marker.lineTo(center.getX() + 2.75, center.getY() - 5.5);
...
// Вывести надпись в точке с вычисленными координатами
		...
		Point2D.Double labelPos = xyToPoint(0.5, -1);
//Вывести надпись в точке с вычисленными координатами
		canvas.drawString("0", (float) (labelPos.getX() - bounds.getWidth() - 10),
				(float) (labelPos.getY() + bounds.getY()));
